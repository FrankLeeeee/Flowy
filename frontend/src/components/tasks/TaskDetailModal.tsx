...
import {
  getHarnessConfigBadges,
  parseHarnessConfig,
} from "../../lib/harnessConfig";
...
export default function TaskDetailModal({
  ...
  const [labelsText, setLabelsText] = useState<string>(
    task.labels ? (JSON.parse(task.labels || "[]") as string[]).join(", ") : "",
  );
  ...
  const loadLogs = useCallback(async () => {
    try {
      setLogs(await fetchTaskLogs(task.id));
    } catch {
      /* ignore */
    }
  }, [task.id]);
  ...
  const canViewFullscreenOutput =
    hasOutput && TERMINAL_STATUSES.includes(task.status);
  const harnessConfigBadges = getHarnessConfigBadges(
    task.ai_provider,
    parseHarnessConfig(task.harness_config),
  );
  ...
  return (
    ...
    >
      <AppDialogContent className="sm:max-w-2xl max-h-[90vh] gap-0 flex flex-col">
        <AppDialogHeader>
          ...
          <div className="flex items-center gap-2.5 flex-wrap text-[11px] font-medium">
            <span className="text-[11px] font-mono tracking-wide text-muted-foreground/75">
              {task.task_key}
            </span>
            <span
              className={cn(
          ...
          <div className="flex flex-col gap-5 px-6 py-5 min-w-0 overflow-x-hidden">
            ...
            {editing ? (
                    ...
                    {labels.map((label) => {
                      ...
                      return (
                        <Badge
                          ...
                          variant="secondary"
                          className={cn(
                            "gap-1 rounded-full px-2.5 py-1 text-[10px] font-medium ring-1",
                            colorStyles.pill,
                          )}
                        >
                          {label}
                    ...
                    <SelectContent className="rounded-xl border-border/60 bg-popover p-1 shadow-none">
                      {STATUS_OPTIONS.map((item) => (
                        <SelectItem
                          key={item.value}
                          value={item.value}
                          className="rounded-lg py-2 pl-8 pr-3 text-[11px] font-medium"
                        >
                          <span
                            className={cn(
                              "inline-flex items-center gap-2",
                              getTaskStatusStyles(item.value).text,
                            )}
                          >
                            <span
                              className={cn(
                                "h-2 w-2 rounded-full",
                                getTaskStatusStyles(item.value).dot,
                              )}
                            />
                            {item.label}
                          </span>
                      ...
                      {PRIORITY_OPTIONS.map((item) => (
                        ...
                        >
                          <span
                            className={cn(
                              "inline-flex items-center gap-2",
                              getTaskPriorityStyles(item.value).text,
                            )}
                          >
                            <span
                              className={cn(
                                "h-2 w-2 rounded-full",
                                getTaskPriorityStyles(item.value).dot,
                              )}
                            />
                            {item.label}
                          </span>
              ...
              <div>
                ...
                {task.description && (
                  <p className="mt-2 whitespace-pre-wrap text-[13px] leading-relaxed text-muted-foreground/90">
                    {task.description}
                  </p>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] text-muted-foreground/75">
              ...
              {task.started_at && (
                <div>Started: {fmtTime(task.started_at)}</div>
              )}
              {task.completed_at && (
                <div>Completed: {fmtTime(task.completed_at)}</div>
              )}
            </div>
      ...
      <Dialog
        ...
        onOpenChange={setOutputFullscreenOpen}
      >
          ...
          <AppDialogHeader className="pr-14">
            ...
            <div className="flex items-center gap-2.5 text-[11px] font-medium text-muted-foreground/75">
              ...
              >
                {statusConfig.icon}
                {
                  STATUS_OPTIONS.find((item) => item.value === task.status)
                    ?.label
                }
              </span>
            </div>
            ...
            <div className="w-full px-6 py-6">
              <div
                className={cn(
                  ...
                  "min-h-full bg-background shadow-soft",
                )}
              >
                <ReactMarkdown remarkPlugins={[remarkGfm]}>
                  {output}
