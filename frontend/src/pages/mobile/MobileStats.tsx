...
function StatPill({
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center rounded-xl px-3 py-3 ring-1 flex-1",
        styles.panel,
      )}
    >
      <span
...
function MiniBar({
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="min-w-[90px] shrink-0 truncate text-[12px] text-muted-foreground/85">
        {label}
      </span>
      <div className="flex-1 h-1.5 rounded-full bg-foreground/[0.06] overflow-hidden">
        <div
          ...
          style={{
            width: `${pct}%`,
            backgroundColor: dotColor ?? "var(--primary)",
          }}
        />
      </div>
      <span className="w-6 shrink-0 text-right text-[11px] font-semibold tabular-nums text-foreground/80">
        {count}
      </span>
    </div>
  );
...
function ActivityChart({
  ...
  return (
    <div className="flex items-end gap-0.5 h-12">
      {grid.map(({ date, count }) => {
        const heightPct =
          max > 0 ? Math.max((count / max) * 100, count > 0 ? 10 : 5) : 5;
        return (
          <div
            key={date}
            className="flex-1"
      ...
      <div className="p-4 space-y-4 pb-6">
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          ...
          {dailyCompleted.length === 0 ? (
            <p className="text-[12px] text-muted-foreground/60 py-4 text-center">
              ...
              tasksByList.map(({ list_name, total, done }) => (
                <div key={list_name} className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-[12px] text-muted-foreground/85 truncate max-w-[160px]">
                      {list_name}
                    </span>
                  ...
                  <div className="h-1.5 rounded-full bg-foreground/[0.06] overflow-hidden">
                    <div className="flex h-full rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${maxListCount > 0 ? (done / maxListCount) * 100 : 0}%`,
                        }}
                      />
                      <div
                        className="h-full bg-primary/30"
                        style={{
                          width: `${maxListCount > 0 ? ((total - done) / maxListCount) * 100 : 0}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          ...
          {tasksByProvider.length === 0 ? (
            <p className="text-[12px] text-muted-foreground/60">
              No tasks assigned to a provider yet.
            </p>
          ) : (
            <div className="space-y-2.5">
              {tasksByProvider.map(({ ai_provider, count }) => {
                ...
                const label =
                  AI_LABELS[ai_provider as AiProvider] ?? ai_provider;
                return (
                  <MiniBar
                    key={ai_provider}
                    label={label}
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-4">
            ...
            <div className="space-y-2.5">
              {tasksByRunner.map(({ runner_name, count, runner_status }) => {
                const statusStyles = getRunnerStatusStyles(
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-4">
            ...
            <div className="space-y-2.5">
              {topLabels.map(({ name, color, count }) => {
                const colorStyles =
                  LABEL_COLORS[color as LabelColor] ?? LABEL_COLORS["gray"];
                return (
                  <MiniBar
                    key={name}
                    label={name}
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-4">
          ...
          <div className="flex gap-2">
            {[
              {
                label: "Online",
                value: runnerCounts.online,
                tone: "success" as const,
              },
              {
                label: "Busy",
                value: runnerCounts.busy,
                tone: "warning" as const,
              },
              {
                label: "Offline",
                value: runnerCounts.offline,
              ...
              return (
                <div
                  key={label}
                  className={cn(
                    "flex flex-col items-center rounded-xl px-3 py-2.5 ring-1 flex-1",
                    s.panel,
                  )}
                >
                  <span
                  ...
                  <span
                    className={cn("mt-0.5 text-[10px] font-medium", s.text)}
                  >
                    {label}
                  </span>
                </div>
